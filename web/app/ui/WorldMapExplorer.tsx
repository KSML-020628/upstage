...
export function WorldMapExplorer() {
  ...
  const panLimits=(value:number)=>({x:(typeof window==="undefined"?700:window.innerWidth*.62)*(value-1),y:(typeof window==="undefined"?400:window.innerHeight*.55)*(value-1)});
  const changeZoom=(next:number)=>{
    const value=Math.min(2.5,Math.max(1,Math.round(next*10)/10));
    zoomRef.current=value;setZoom(value);
    if(value===1)setPan({x:0,y:0});
    else { const limit=panLimits(value);setPan((current)=>({x:Math.max(-limit.x,Math.min(limit.x,current.x)),y:Math.max(-limit.y,Math.min(limit.y,current.y))})); }
  };
  ...
  const selectContinent=(name:ContinentName)=>{
    ...
    const x=-(((point?.[0]??WIDTH/2)/WIDTH)-.5)*width*targetZoom;
    const y=-(((point?.[1]??HEIGHT/2)/HEIGHT)-.5)*height*targetZoom;
    zoomRef.current=targetZoom;setZoom(targetZoom);setPan({x:Math.max(-limit.x,Math.min(limit.x,x)),y:Math.max(-limit.y,Math.min(limit.y,y))});setContinent(name);setCountry(null);
  };
  ...
  return <main className="world-explorer">
    ...
    {!continent && <section className="world-copy"><p>EXPLORE BEFORE YOU DECIDE</p><h1>어디로 갈지 몰라도 괜찮아요. <em>대륙부터</em> 천천히 둘러보세요.</h1><div className="krw-rate-strip"><small>대한민국 원화 기준</small>{rateCurrencies.map((currency)=><span key={currency}><b>{currency}</b> {krwRates[currency]?`${Math.round(krwRates[currency]).toLocaleString("ko-KR")}원`:"—"}</span>)}</div></section>}
    <div className={`flat-world ${panning?"is-panning":""}`} aria-label="대륙별 교환대학 세계 지도" onWheel={(event)=>{event.preventDefault();changeZoom(zoomRef.current+(event.deltaY<0?.2:-.2));}} onPointerDown={(event)=>{if((event.target as HTMLElement).closest("button"))return;dragRef.current={x:event.clientX,y:event.clientY,panX:pan.x,panY:pan.y};setPanning(true);event.currentTarget.setPointerCapture(event.pointerId);}} onPointerMove={(event)=>{if(!dragRef.current)return;const limit=panLimits(zoomRef.current);setPan({x:Math.max(-limit.x,Math.min(limit.x,dragRef.current.panX+event.clientX-dragRef.current.x)),y:Math.max(-limit.y,Math.min(limit.y,dragRef.current.panY+event.clientY-dragRef.current.y))});}} onPointerUp={(event)=>{dragRef.current=null;setPanning(false);if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);}} onPointerCancel={()=>{dragRef.current=null;setPanning(false);}}>
      <div className="world-map-stage" style={{transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`}}>
        <div className="world-map-canvas" ref={canvasRef}>
          ...
          {continentOrder.map((name)=>{const count=[...groups.keys()].filter((item)=>continentFor(item)===name).length;return <button type="button" key={name} className={`continent-pin ${continent===name?"active":""}`} style={continentPosition(name)} onClick={()=>selectContinent(name)}><b>{name}</b><span>{count?`${count}개 국가`:"준비 중"}</span></button>;})}
        </div>
      </div>
