...
export function InteractiveGlobe({ universities, onCountryClick }: { universities: University[]; onCountryClick:(group:CountryGroup)=>void }) {
  ...
  useEffect(() => {
    ...
    const animate=(time:number) => {
      if (!dragRef.current && zoomRef.current === 1 && time-last>32) { rotationRef.current=(rotationRef.current+.16)%360; setRotation(rotationRef.current); last=time; }
  ...
  const changeZoom = (next: number) => {
    const value = Math.min(2.2, Math.max(1, Math.round(next * 10) / 10));
  ...
  const projection=geoOrthographic().translate([SIZE/2,SIZE/2]).scale(330*zoom).rotate([rotation,tilt,0]).clipAngle(90).precision(.35);
  ...
  return <div className={`interactive-globe ${dragging?"is-dragging":""}`} onWheel={(event)=>{event.preventDefault();changeZoom(zoomRef.current+(event.deltaY<0?.2:-.2));}} onPointerDown={(event)=>{if((event.target as HTMLElement).closest("button"))return;dragRef.current={x:event.clientX,y:event.clientY,rotation:rotationRef.current,tilt:tiltRef.current};setDragging(true);event.currentTarget.setPointerCapture(event.pointerId);}} onPointerMove={(event)=>{if(!dragRef.current)return;rotationRef.current=(dragRef.current.rotation+(event.clientX-dragRef.current.x)/2.2)%360;tiltRef.current=Math.max(-80,Math.min(80,dragRef.current.tilt-(event.clientY-dragRef.current.y)/2.2));setRotation(rotationRef.current);setTilt(tiltRef.current);}} onPointerUp={(event)=>{dragRef.current=null;setDragging(false);if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);}} onPointerCancel={()=>{dragRef.current=null;setDragging(false);}}>
